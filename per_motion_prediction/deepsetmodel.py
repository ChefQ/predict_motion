...
class DeepSetLLM(nn.Module):
    """
    DeepSet model for predicting the likelihood a motion will be granted or denied
    The model is composed of two parts:
    1. A feature extractor that processes the input data. The feature extractor is a Longformer LLM from Hugging Face.
    2. A feedforward neural network with two hidden layers.
    """

    def __init__(self, LLM, hidden_size, classify_size):
        super(DeepSetLLM, self).__init__()
        
        self.phi = LLM
        self.rho = nn.Sequential(
            nn.Linear(hidden_size, classify_size),
            nn.ReLU(inplace=True),
            nn.Linear(classify_size, 1),
            nn.Sigmoid()
        )
